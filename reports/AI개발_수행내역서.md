...
<!-- [HWP: Bold] R²/RMSE/MAE 행에서 랜덤포레스트 수치(0.9062, 0.9967, 0.7735)에 Bold 적용 -->
...
초기 n_estimators=200 기준 pkl 직렬화 파일이 100MB 이상으로 Streamlit Cloud 배포 한도를 초과하였다. <!-- [HWP: Bold] n_estimators=100, max_depth=15 / 13.82MB / 최종 배포 모델 에 Bold 적용 -->
이에 **n_estimators=100, max_depth=15, max_features="sqrt"**로 하이퍼파라미터를 조정하고, joblib **compress=3**(무손실 압축)을 적용하여 pkl 크기를 **13.82MB**로 줄였다. 조정 후 R² = 0.9011로 근소하게 감소하였으나, 랜덤 포레스트의 우수한 앙상블 특성을 유지하며 git 추적 및 Streamlit Cloud cold start 없이 즉시 로드가 가능하다. 이 최적화된 설정이 **최종 배포 모델**(`ml/train.py` 및 `models/loan_rate_model.pkl`)로 채택되었다.
...
<!-- [HWP: 초록+Bold] 아래 세 수치 1.0235 / 0.7941 / 0.9011 에 적용 -->
...
4종 비교 실험 결과 최우수 성능을 기록한 **XGBClassifier**를 최종 배포 모델로 채택하였다. n_estimators=100, compress=3 적용으로 pkl 크기는 **0.1MB**에 불과하다.

<!-- [HWP: 초록+Bold] AUC-ROC 행 수치 0.9528 에 적용 -->
<!-- [HWP: 표-AUC-ROC 행 배경색 연노랑 적용] -->
| 지표 | 정상(0) | 부도(1) |
|---|---|---|
| Precision | 0.93 | 0.97 |
| Recall | 0.99 | 0.75 |
...
- **AUC-ROC 0.9528**: 랜덤 분류기(0.5) 대비 월등히 높은 수치로, 부도 여부를 높은 정확도로 구별할 수 있음을 의미한다.
- **Recall(재현율) 부도=0.75**: 실제 부도 건 중 75%를 정확히 탐지한다. RF(0.71) 대비 4%p 향상으로, 금융 리스크 관리 측면에서 부도 미탐지(False Negative)가 줄었다.
- **Precision 부도=0.97**: 부도로 예측한 건 중 97%가 실제 부도로, 오경보(False Alarm) 비율이 매우 낮다.
...
<!-- [HWP: Bold] 대출 등급(loan_grade) / R² 0.9011 / RMSE 1.0235 / MAE 0.7941 / 1%p 이내 에 Bold 적용 -->
...
| | `ml/train.py` | RF 회귀(n_estimators=100, max_depth=15, compress=3) + XGBoost 분류기 학습(n_estimators=100, compress=3), pkl 저장 |
