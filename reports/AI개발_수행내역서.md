...
<!-- [HWP: 표-그래디언트부스팅 열(3번째 열) 전체 배경색 연노랑 적용] -->
<!-- [HWP: Bold] R²/RMSE/MAE 행에서 그래디언트부스팅 수치(0.9038, 1.0093, 0.7906)에 Bold 적용 -->
| 분류 | 선형 회귀 | 릿지 회귀 | 그래디언트 부스팅 | 랜덤 포레스트 |
|---|---|---|---|---|
| 독립변수 | 전체 10개 피처 | 전체 10개 피처 | 전체 10개 피처 | 전체 10개 피처 |
| 종속변수 | 대출금리 | 대출금리 | 대출금리 | 대출금리 |
| R² | 0.8692 | 0.8692 | **0.9038** | 0.9013 |
| RMSE | 1.1770 | 1.1770 | **1.0093** | 1.0224 |
| MAE | 0.9173 | 0.9173 | **0.7906** | 0.7946 |

#### ○ 회귀 모델 선정

비교 실험 결과 그래디언트 부스팅(n_estimators=200, R²=**0.9038**)이 소폭 앞섰으나, 랜덤 포레스트(n_estimators=100, max_depth=15, R²=0.9013)와의 차이는 0.0025로 실무 영향이 미미하다.

배포 모델로 **랜덤 포레스트**를 채택하였다. joblib compress=3(무손실 압축) 적용 시 pkl 크기가 **13.82MB**로 경량화되며, Streamlit Cloud cold start 없이 즉시 로드가 가능하다. 그래디언트 부스팅 n_estimators=200 pkl은 200MB 이상으로 git 추적 및 배포 용량 한도를 초과한다.
...
다수의 결정 트리를 병렬로 학습하여 평균을 취하는 랜덤 포레스트를 적용하였다. 독립변수는 전체 10개 피처를 사용한다. <!-- [HWP: Bold] n_estimators=100, max_depth=15 / 13.82MB / 최종 배포 모델 에 Bold 적용 -->
**n_estimators=100, max_depth=15, max_features="sqrt"** 하이퍼파라미터와 joblib **compress=3**(무손실 압축)을 적용하여 pkl 크기를 **13.82MB**로 경량화하였다. R² = 0.9013으로 그래디언트 부스팅과 근소한 차이이며, 앙상블 안정성과 배포 효율을 갖춘 이 설정이 **최종 배포 모델**(`ml/train.py` 및 `models/loan_rate_model.pkl`)로 채택되었다.
...
<!-- [HWP: 초록+Bold] 아래 세 수치 1.0224 / 0.7946 / 0.9013 에 적용 -->
- RMSE **1.0224** : 예측 금리의 평균 오차 약 1%p 이내
- MAE **0.7946** : 절대 오차 평균 약 0.79%p
- R² **0.9013** : 대출 금리 분산의 90.1% 설명
...
선형 회귀, 릿지 회귀, 그래디언트 부스팅, 랜덤 포레스트 모델을 비교 실험한 결과, 비교 실험 기준 최우수 모델은 그래디언트 부스팅(R²=0.9038)이었다. 그러나 n_estimators=200 기준 pkl 크기가 200MB 이상으로 배포가 불가능하여, **n_estimators=100, max_depth=15, compress=3** 조합의 **랜덤 포레스트**를 최종 배포 모델로 채택하였다(pkl **13.82MB**).

<!-- [HWP: Bold] 대출 등급(loan_grade) / R² 0.9013 / RMSE 1.0224 / MAE 0.7946 / 1%p 이내 에 Bold 적용 -->
<!-- [HWP: 빨강] RandomForestRegressor 단어에 빨강 적용 -->
피처 중요도 분석에서 대출 등급(loan_grade)이 예측에 가장 큰 기여를 하였으며, 대출 금액, 소득 대비 비율 등이 보조적으로 작용하였다. 최종 배포 모델(RandomForestRegressor) 기준 테스트 데이터 성능은 R² **0.9013**, RMSE **1.0224**, MAE **0.7946**로, 대출 금리 분산의 90% 이상을 설명한다. 예측 오차는 평균 약 1%p 이내로 실무 적용 가능한 수준의 정확도를 확보하였으며, compress=3 무손실 압축으로 파일 크기를 대폭 줄여 Cloud 배포 환경에서 안정적으로 운용 가능하다.
...
| | `ml/train.py` | RF 회귀(n_estimators=100, max_depth=15, compress=3), pkl 저장 |
