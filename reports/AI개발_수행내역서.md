...
결론: R² 점수가 가장 높고(0.9062), RMSE 및 MAE 또한 가장 낮은 **랜덤 포레스트 회귀 모델**을 최종 채택하였다.
...
약한 학습기(weak learner)를 순차적으로 결합하는 그래디언트 부스팅을 적용하였다. n_estimators=200, random_state=42로 학습한 결과 R² = 0.9041로 선형 계열 모델 대비 성능이 크게 향상되었다. 예측값이 실제 데이터의 분포를 훨씬 더 잘 따라가는 것을 확인할 수 있다. 다만 순차 학습 방식으로 인해 랜덤 포레스트 대비 학습 속도가 느리다는 단점이 있다.

---

#### - 랜덤 포레스트 회귀 모델 (Random Forest Regressor) — 최종 채택
...
그래디언트 부스팅 대비 R²가 0.0021 높고, RMSE는 0.0112, MAE는 0.0164 낮아 전 지표에서 우세하였다. 또한 랜덤 포레스트는 앙상블 특성상 과적합에 강건하고, 피처 중요도를 직관적으로 해석할 수 있는 장점이 있어 최종 모델로 선정하였다.
...
선형 회귀, 릿지 회귀, 그래디언트 부스팅, 랜덤 포레스트 모델을 비교 실험한 결과, 예측 설명도가 가장 높고 과적합에 강건한 **랜덤 포레스트 회귀 모델**을 최종 채택하였다.

피처 중요도 분석에서 대출 등급(loan_grade)이 예측에 가장 큰 기여를 하였으며, 대출 금액, 소득 대비 비율 등이 보조적으로 작용하였다. 최종 모델은 테스트 데이터 기준 R² **0.9062**, RMSE **0.9967**, MAE **0.7735**를 달성하여 대출 금리 분산의 90% 이상을 설명한다. 예측 오차는 평균 약 1%p 이내로, 실무 적용 가능한 수준의 정확도를 확보하였다.
