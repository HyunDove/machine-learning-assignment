...
- **대출 금액**: 평균 9,583달러로 5,000~10,000달러 구간에 신청자가 집중되어 있다. 10,000달러 이상 구간은 빈도가 감소하나, 20,000달러·25,000달러 근방에서 소규모 피크가 재등장하는 다봉 분포 형태를 보인다. 최대 35,000달러까지 분포하며 전반적으로 우측 꼬리가 긴 분포이다.
- **소득대비비율**: 평균 0.2(20%)로 0.1~0.2 구간에 가장 많이 집중되어 있으며 우측 꼬리가 긴 분포를 보인다. 대다수 신청자가 소득의 30% 이하를 대출 상환에 사용하나, 일부는 0.6 이상으로 재정 부담이 매우 높은 경우도 존재한다.
