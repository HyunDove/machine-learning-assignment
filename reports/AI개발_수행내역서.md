...
#### - 그래디언트 부스팅 회귀 모델 (Gradient Boosting Regressor) — 최종 배포 채택
...
순수 성능 지표에서는 랜덤 포레스트에 근소하게 뒤지나(R² 기준 0.0021 차이), pkl 직렬화 파일 크기가 **363KB**로 매우 작아 Streamlit Cloud 배포 및 git 추적이 가능하다. 이 실용적 이점으로 인해 **최종 배포 모델**로 선정하였다(`ml/train.py` 및 `models/loan_rate_model.pkl`).

---

#### - 랜덤 포레스트 회귀 모델 (Random Forest Regressor) — 성능 최우수 (배포 미채택)

![랜덤 포레스트 예측 결과](figures/07_model_rf.png)

다수의 결정 트리를 병렬로 학습하여 평균을 취하는 랜덤 포레스트를 적용하였다. 독립변수는 전체 11개 피처를 포함한다. n_estimators=200, random_state=42 설정에서 R² = **0.9062**로 비교 모델 중 가장 높은 성능을 기록하였다.

그래디언트 부스팅 대비 R²가 0.0021 높고, RMSE는 0.0112, MAE는 0.0164 낮아 전 지표에서 우세하다. 그러나 n_estimators=200 기준 pkl 직렬화 파일이 100MB 이상으로 증가하여 Streamlit Cloud 저장소 용량 한도를 초과하고 git 추적이 불가능하였다. 이로 인해 성능 비교 실험에는 포함하되, 실제 배포 모델에서는 제외하였다.
...
위 그래프는 랜덤 포레스트 모델로 테스트 데이터(전체의 20%)를 예측한 결과이다. 대부분의 예측값이 완벽한 예측선(빨간 점선, y=x) 주변에 밀집하여 실제값과 높은 일치를 보인다.

- RMSE **0.9967** : 예측 금리의 평균 오차가 약 1%p 이내
- MAE **0.7735** : 절대 오차 평균 약 0.77%p
- R² **0.9062** : 대출 금리 분산의 90.6% 설명
...
선형 회귀, 릿지 회귀, 그래디언트 부스팅, 랜덤 포레스트 모델을 비교 실험한 결과, 순수 성능 최우수 모델은 랜덤 포레스트(R²=0.9062)였으나 배포 환경의 파일 크기 제약으로 인해 **그래디언트 부스팅 회귀 모델(GBR)**을 최종 배포 모델로 채택하였다.

피처 중요도 분석에서 대출 등급(loan_grade)이 예측에 가장 큰 기여를 하였으며, 대출 금액, 소득 대비 비율 등이 보조적으로 작용하였다. 최종 배포 모델(GBR) 기준 테스트 데이터 성능은 R² **0.9041**, RMSE **1.0079**, MAE **0.7899**로, 대출 금리 분산의 90% 이상을 설명한다. 예측 오차는 평균 약 1%p 이내로 실무 적용 가능한 수준의 정확도를 확보하였으며, pkl 크기가 363KB로 경량화되어 Cloud 배포 환경에서 안정적으로 운용 가능하다.
...
| | `ml/train.py` | GBR 회귀 + RF 분류기 학습, pkl 저장 |
...
| | `models/*.pkl` | 학습 모델 파일 git 추적 — GBR 회귀(363KB), RF 분류기 n_estimators 50으로 경량화(67MB→17MB) |
